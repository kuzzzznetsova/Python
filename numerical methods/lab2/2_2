...
def simple_iter(e, q, a, b):
    ...
    diff_fi = [dfix1(a, b)[0], dfix2(a, b)[0],
               dfix1(a, b)[1], dfix2(a, b)[1]]

    diff_fi = [abs(diff_entry) for diff_entry in diff_fi]
    q = max(diff_fi)

    if (norm(diff_fi) < 1):
        print("Достаточное условие сходимости итерационного процесса выполнено.")
    else:
        print("Достаточное условие сходимости итерационного процесса не выполнено.")
