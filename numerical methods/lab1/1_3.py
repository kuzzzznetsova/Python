...
def simple_iteration(alfa, beta):
    # X^(k)= alpha * X + betta
    e = 0.00001
    roots0 = np.zeros(len(beta))  # корни
...
def main(A):
    ...
    print('Метод простых итераций и метод Зейделя')
    ...
    if k1 < k2:
        k = k2 - k1
        print("Методом простых итераций потребовалось ", k1, " итераций, ", "а методом Зейделя потребовалось ", k2,
              " итераций\n"
...
A = np.array([[29.0, 8.0, 9.0, -9.0, 197.0],
              [-7.0, -25.0, 0, 9.0, -226.0],
              [1.0, 6.0, 16.0, -2.0, -95.0],
              [-7.0, 4.0, -2.0, 17.0, -58.0]])
main(A)
